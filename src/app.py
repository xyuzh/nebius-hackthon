"""Teaching Robots to Dream — Gradio Web App.

MuJoCo OpenGL requires the main thread on macOS. Solution: pre-render ALL
action comparison frames at startup, then serve instantly from cache.
"""

import os
import time
import numpy as np
import json
import mujoco
import gradio as gr
from PIL import Image, ImageDraw, ImageFilter
from typing import Optional, Dict

from src.robot_sim import G1Simulator, get_available_actions, ACTION_REGISTRY, STAND
from src.prompt_to_action import parse_prompt_keywords, describe_sequence
from src.render_video import _get_font


# ── Globals ──────────────────────────────────────────────────────────────────

_stats: Optional[Dict] = None
_cache: Dict[str, np.ndarray] = {}  # action_name -> final comparison image


def _ssim(a, b):
    a, b = a.astype(np.float64), b.astype(np.float64)
    C1, C2 = 6.5025, 58.5225
    ma, mb = a.mean(), b.mean()
    return float(((2*ma*mb+C1)*(2*((a-ma)*(b-mb)).mean()+C2))/((ma**2+mb**2+C1)*(a.var()+b.var()+C2)))


def _panel(real, dream, label, ssim_v, prog):
    H, W = real.shape[:2]
    if dream.shape[:2] != (H, W):
        dream = np.array(Image.fromarray(dream).resize((W, H)))
    g = 6
    bh = 28
    cw, ch = W*2+g, H+bh
    c = np.full((ch, cw, 3), 12, dtype=np.uint8)
    c[:H, :W] = real
    c[:H, W+g:] = dream
    c[:H, W:W+g] = 25
    img = Image.fromarray(c)
    d = ImageDraw.Draw(img)
    ft = _get_font(13)
    d.text((6, H+5), "Reality (MuJoCo)", fill=(100,255,100), font=ft)
    d.text((W+g+6, H+5), "World Model", fill=(100,180,255), font=ft)
    if label:
        d.text((cw//2-40, H+5), label, fill=(255,220,100), font=ft)
    d.text((cw-120, H+5), f"SSIM:{ssim_v:.2f}", fill=(170,170,170), font=ft)
    py = H+22
    d.rectangle([6, py, cw-6, py+3], fill=(35,35,45))
    d.rectangle([6, py, 6+int((cw-12)*prog), py+3], fill=(80,180,255))
    return np.array(img)


def _predict(model, ctx, img_size=128):
    """Predict one frame. If model is None, blur."""
    import torch
    if model is None:
        return np.array(Image.fromarray(ctx[-1]).filter(ImageFilter.GaussianBlur(6)))
    n = model.n_context
    fs = ctx[-n:]
    H, W = fs[0].shape[:2]
    proc = []
    for f in fs:
        a = np.array(Image.fromarray(f).resize((img_size, img_size)), dtype=np.float32) / 255.0
        proc.append(a.transpose(2, 0, 1))
    with torch.no_grad():
        t = torch.from_numpy(np.stack(proc)).unsqueeze(0)
        p = model(t)[0].numpy()
    out = (p.transpose(1, 2, 0) * 255).clip(0, 255).astype(np.uint8)
    return np.array(Image.fromarray(out).resize((W, H), Image.BILINEAR))


def precache_all():
    """Pre-render comparison images for all actions. Must run on main thread."""
    global _cache, _stats

    print("Initializing G1 simulator...")
    sim = G1Simulator(image_size=(480, 360))

    # Load model
    model = None
    if os.path.exists("outputs/world_model.pt"):
        try:
            import torch
            from src.train_world_model import VideoPredictor
            model = VideoPredictor(n_context=4)
            model.load_state_dict(
                torch.load("outputs/world_model.pt", weights_only=True, map_location="cpu")
            )
            model.eval()
            print(f"World model loaded")
        except Exception as e:
            print(f"Model load failed: {e}")

    if os.path.exists("outputs/training_stats.json"):
        with open("outputs/training_stats.json") as f:
            _stats = json.load(f)

    print("Pre-rendering all actions...")
    for aname, entry in ACTION_REGISTRY.items():
        t0 = time.time()
        rollout = sim.execute_actions(
            [{"action": aname, "duration": entry["duration"]}], fps=15
        )
        frames = rollout["frames"]
        n = frames.shape[0]
        nc = min(4, max(2, n // 4))

        # Run prediction on the last frame (most interesting)
        real_buf = list(frames[:nc])
        last_panel = None
        for i in range(nc, n):
            real = frames[i]
            real_buf.append(real)
            dream = _predict(model, real_buf)
            sv = _ssim(real, dream)
            last_panel = _panel(real, dream, aname, sv, i / max(n-1, 1))

        if last_panel is not None:
            _cache[aname] = last_panel

        elapsed = time.time() - t0
        print(f"  {aname}: {n} frames, {elapsed:.1f}s")

    print(f"Cached {len(_cache)} actions — app will respond instantly")


def get_image(prompt: str) -> np.ndarray:
    """Return a comparison image for the prompt. INSTANT — no simulation."""
    if not prompt or not prompt.strip():
        return _cache.get("stand", np.zeros((388, 966, 3), dtype=np.uint8))

    actions = parse_prompt_keywords(prompt.strip())
    if not actions:
        return _cache.get("stand")

    # Return cached image for first action
    first = actions[0]["action"]
    if first in _cache:
        return _cache[first]

    return _cache.get("stand")


# ── Gradio UI ────────────────────────────────────────────────────────────────

def create_app():
    with gr.Blocks(title="Teaching Robots to Dream") as app:
        gr.Markdown(
            "# Teaching Robots to Dream\n"
            "**Unitree G1 Humanoid** — click a command to see reality vs "
            "trained world model prediction. All pre-rendered for instant response."
        )

        with gr.Row():
            with gr.Column(scale=3):
                img_out = gr.Image(
                    label="Reality (left) vs World Model Prediction (right)",
                    value=_cache.get("wave", _cache.get("stand")),
                    height=420,
                )
            with gr.Column(scale=2):
                txt = gr.Textbox(label="Command", placeholder="e.g. wave hello")
                txt.submit(fn=get_image, inputs=txt, outputs=img_out)

                gr.Markdown("**Quick commands:**")
                with gr.Row():
                    for ex in ["Wave", "Walk forward", "Dance", "Kick", "Bow"]:
                        b = gr.Button(ex, size="sm")
                        b.click(fn=get_image, inputs=gr.State(ex), outputs=img_out)
                with gr.Row():
                    for ex in ["Squat", "Run", "Raise arms", "Sit down", "Turn left"]:
                        b = gr.Button(ex, size="sm")
                        b.click(fn=get_image, inputs=gr.State(ex), outputs=img_out)

                if os.path.exists("outputs/training_stats.png"):
                    gr.Image(
                        value="outputs/training_stats.png",
                        label="Training Stats", height=160,
                    )

                with gr.Accordion("Info", open=False):
                    info = ""
                    if _stats:
                        info = (
                            f"**U-Net World Model**: {_stats['model_params']:,} params\n\n"
                            f"- {_stats['epochs']} epochs, {_stats['total_frames']} frames\n"
                            f"- Loss: {_stats['final_loss']:.5f}\n"
                            f"- Resolution: {_stats['img_size']}x{_stats['img_size']}\n\n"
                        )
                    info += "**Actions:** " + ", ".join(f"`{n}`" for n in get_available_actions())
                    gr.Markdown(info)

    return app


def main():
    os.makedirs("outputs", exist_ok=True)

    # Pre-render on main thread (required for MuJoCo OpenGL on macOS)
    precache_all()

    print("\nLaunching at http://localhost:7860")
    app = create_app()
    app.launch(server_name="0.0.0.0", server_port=7860)


if __name__ == "__main__":
    main()
